...
(def test-index {:id "test" :map (fn [doc] {"author" (doc :author)}) :storage (lucene/create-memory-index) })

(def test-indexes [ test-index ])
...
;; Bugger, I need to do this the Couch way
;; invoke a map function for every value I want to return
;; Or allow lists
;; 
(describe "loading indexes from the database and indexing using them"
  (it "will index all the documents"
    (with-db (fn [db]
        (write-three-documents db)
        (with-open [tx (.ensure-transaction db)]
          (.commit! (indexes/put-index tx { :id "by_author" :map "(fn [doc] {\"author\" (doc :author)})"})))
