...
(defn add-standard-data-set [instance]
  (db/put-document instance "docs-1" { "name" "zebra" "description" "the zebra is a horse like creature with black and white stripes" "number" 25} {})
      ...
      (fn [{:keys [storage index-engine] :as instance}]
        ...
        (should= "zebra" 
          (extract-name-from-result 
            (db/query instance  {:index "default" :filter (=? "name" "zebra")}))))))
   (it "will search on partial short strings"
     (with-full-setup
      (fn [{:keys [storage index-engine] :as instance}]
        ...
        (should= "zebra" 
          (extract-name-from-result 
            (db/query instance {:index "default" :filter (starts-with? "name" "zeb")}))))))
    ...
    (it "will do partial-word-based matching on long strings"
     (with-full-setup
      (fn [{:keys [storage index-engine] :as instance}]
        ...
        (should= "anteater" 
          (extract-name-from-result 
            (db/query instance {:index "default" :filter (has-word-starting-with? "description" "stup")}))))))
     ...
     (with-full-setup
      (fn [{:keys [storage index-engine] :as instance}]
        ...
        (should= "anteater" 
          (extract-name-from-result 
            (db/query instance  {:index "default" :filter (=? "number" 50)} ) )))))
     ...
     (with-full-setup
      (fn [{:keys [storage index-engine] :as instance}]
        ...
        (should= "zebra" 
          (extract-name-from-result 
            (db/query instance {:index "default" :filter (<? "number" 30)}) )))))
      ...
      (fn [{:keys [storage index-engine] :as instance}]
          ...
          (should= "aardvark" 
            (extract-name-from-result 
              (db/query instance  {:index "default" :filter (>? "number" 499)} ) )))))
     ...
     (with-full-setup
      (fn [{:keys [storage index-engine] :as instance}]
        ...
        (indexing/wait-for-index-catch-up storage 50)
        (should= 1 
          (count (db/query instance  {:index "default" :filter (<? "number" 50)} ))))))
      ...
      (it "will exclude the literal from the greater than range "
        (with-full-setup
          (fn [{:keys [storage index-engine] :as instance}]
            ...
            (indexing/wait-for-index-catch-up storage 50)
            (should= 1
              (count (db/query instance  {:index "default" :filter (>? "number" 100)} ) )))))
      ...
      (it "will do less than or equal than matching on integers"
        (with-full-setup
          (fn [{:keys [storage index-engine] :as instance}]
            ...
            (should= "zebra" 
              (extract-name-from-result 
                (db/query instance  {:index "default" :filter (<=? "number" 25)} ) )))))
        ...
        (it "will do greater than or equal than matching on integers"
         (with-full-setup
          (fn [{:keys [storage index-engine] :as instance}]
            ...
            (should= "aardvark" 
              (extract-name-from-result 
                (db/query instance  {:index "default" :filter (>=? "number" 500)} ) )))))
        ...
        (with-full-setup
          (fn [{:keys [storage index-engine] :as instance}]
            ...
            (should== { :name "Air" :album "Talkie Walkie" } 
                      (first 
                        (db/query instance  {:index "default" :filter (=? :name "Air")} ) )))))

      (it "will allow queries inside collections"
        (with-full-setup
          (fn [{:keys [storage index-engine] :as instance}]
            (db/put-document instance "1" { "name" "bob" :collection [ "one" "two" "three"]}{})
            (db/put-document instance "2" { "name" "alice" :collection [ "two" "three" "four"]} {})
            (indexing/wait-for-index-catch-up storage 50)
            (should= "bob"
              (extract-name-from-result
                (db/query instance {:index "default" :filter (has-item? :collection "one")}))))))
      ...
      (it "will allow multiple claused joined by an 'and'"
        (with-full-setup
          (fn [{:keys [storage index-engine] :as instance}]
            (db/put-document instance "1" { "name" "bob" :collection [ "one" "two" "three"]} {})
            (db/put-document instance "2" { "name" "alice" :collection [ "two" "three" "four"]} {})
            (db/put-document instance "3" { "name" "alice" :collection [ "one" "four"]} {})
            (indexing/wait-for-index-catch-up storage 50)
            (should== { "name" "alice" :collection [ "two" "three" "four"]}
              (first (db/query instance  { :index "default" :filter (AND (=? "name" "alice") (has-item? :collection "two"))} ) )))))  
      ...
      (it "will allow multiple claused joined by an 'or'"
        (with-full-setup
          (fn [{:keys [storage index-engine] :as instance}]
            ...
            (indexing/wait-for-index-catch-up storage 50)
            (should= 3
              (count (db/query instance  {:index "default" :filter (OR (has-item? :collection "four") (has-item? :collection "two")) })))))) 
      ...
      (it "will not include results when using a 'not'"
        (with-full-setup
          (fn [{:keys [storage index-engine] :as instance}]
            ...
            (should= "bob"
              (extract-name-from-result
                (db/query instance  {:index "default" :filter (NOT (=? "name" "alice"))} ))))))
